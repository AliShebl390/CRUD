...
var searchValue = document.getElementById("searchVal");
var products = [];
...
function display() {
    ...
    for (var i = 0; i < products.length; i++) {
        ...
        <td> <button onclick="getToUpdateProduct(` +i +`)" class="btn btn-warning"> Update</button> </td>        <td> <button onclick="deleteProduct(` +i +`)" class="btn btn-danger"> Delete</button> </td>
    </tr>`;
    }
...
function search() {
    ...
    for (var i = 0; i < products.length; i++) {
        if (products[i].name.toLowerCase().includes(searchVal) || products[i].category.toLowerCase().includes(searchVal)){
        if (products[i].sale) {
            productSaleValue = "Yes";
        } else {
            productSaleValue = "No";
        ...
        result +=
            `<tr>
            <td>` +            (i + 1) +`</td>
    <td>` +products[i].name.toLowerCase().replace(searchVal,"<span class='bg-info'>"+searchVal+"</span>") +`</td>
        <td>` +    products[i].price +`</td>
        <td>` +products[i].category.toLowerCase().replace(searchVal,"<span class='bg-info'>"+searchVal+"</span>") +`</td>
        <td>` +productSaleValue +`</td>
        <td>` +products[i].description +`</td>
        <td> <button onclick="getToUpdateProduct(` +i +`)" class="btn btn-warning"> Update</button> </td>        <td> <button onclick="deleteProduct(` +i +`)" class="btn btn-danger"> Delete</button> </td>
    </tr>`;
